...
using Gameplay.Systems.Ability;
using HarmonyLib;
...
[HarmonyPatch]
public unsafe class FreezeFixPatch3
{
    static Type HookedSystemType = typeof(DebugEventsSystem);
    ...
    public static void BeforeUnPatch()
    {
        if (_initialized) {
    ...
    static bool PerformanceRecorderSystem_OnUpdate_Prefix(PerformanceRecorderSystem __instance)
    {
        ...
        {
            LogUtil.LogError("null sytem type index");
        }
        else
        {
            LogUtil.LogMessage($"hooked system: {TypeManager.GetSystemType(_hookedSystemTypeIndex)}");
        }
        _initialized = true;
    ...
    [HarmonyPrefix]
    static void PerformanceRecorderSystem_EndSystem_Prefix(SystemState* systemState)
    {
        if (systemState->m_SystemTypeIndex == _hookedSystemTypeIndex)
        {
            //OnUpdatePrefix();
    ...
    {
        // return if no longer valid
        if (!entityManager.HasComponent<DamageTakenEvent>(eventEntity))
        {
            return;
        }
        ...
        // return if no longer valid
        if (!entityManager.HasComponent<EntityOwner>(damageTaken.Source))
        {
            return;
        }
        var attackerEntity = entityManager.GetComponentData<EntityOwner>(damageTaken.Source);

        // return if not a player attacking
        ...
        {
            // only if they frost dash attacked in the prev frame
            if (!FrostDashAttackersFromPreviousUpdate.Contains(attackerEntity))
            {
                // todo: turn back on
                //return;
            }
            ...
            {
                LogUtil.LogMessage("APPLYING FIX!");
